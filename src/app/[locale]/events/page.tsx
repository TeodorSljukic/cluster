...
async function getEvents(locale: Locale) {
  try {
    ...
    console.log(`[Events] Found ${posts.length} published event posts`);
...
export default async function EventsPage({
  ...
  const posts = await getEvents(locale);
  ...
  return (
    <main className="blog-archive container">
      <h2 data-aos="fade-up">{t.eventsPage.title}</h2>
